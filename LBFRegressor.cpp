...
void LBFRegressor::Save(string path,string name){
    cout << endl<<"Saving model..." << endl;
    ofstream fout;
    fout.open(path+name);
    WriteGlobalParam(fout);
	string RegressorPath = path;
    WriteRegressor(RegressorPath,fout);
    fout.close();
    cout << "End" << endl;    
}

void LBFRegressor::Load(string path,string name){
    cout << "Loading model..." << endl;
    ifstream fin;
    fin.open( path+"/model/"+name );
    ReadGlobalParam(fin);
    ReadRegressor(path,fin);
    fin.close();
    cout << "End"<<endl;
}
...
void  LBFRegressor::WriteRegressor(string Rpath,ofstream& fout){
    for(int i = 0;i < global_params.landmark_num;i++){
        fout << mean_shape_(i,0)<<" "<< mean_shape_(i,1)<<" ";
    }
    fout<<endl;
    ofstream fout_reg;
    fout_reg.open(Rpath + "Regressor.model",ios::binary);
    for (int i=0; i < global_params.max_numstage; i++ ){
...
void LBFRegressor::ReadRegressor(std::string path,ifstream& fin){
    ...
    fin_reg.open(path +modelPath+ "Regressor.model",ios::binary);
    for (int i=0; i < global_params.max_numstage; i++ ){
